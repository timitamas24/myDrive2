...
class DbUtil {
    ...
    setFolderLock = async(folderID: string, lock: Date) => {
        const folder = await Folder.findOneAndUpdate({"_id": new ObjectID(folderID)}, {"$set": {"lockDate": lock}}) as FolderInterface;

        return folder;
    }

    getLockedFolders = async(userID: string) => {
        const folders = await Folder.find({"owner": userID}) as FolderInterface[];
        const foldersID : string[] = folders.map(fld => {
            console.log("the ids: " + fld._id);
            if(fld.lockDate != undefined) {
                let ts = Date.now();
                let currDate = new Date(ts);

                if(fld.lockDate > currDate){
                    return fld._id;
                }
            }
        })
