...
class FolderService {
    ...
    lockFolder = async(folderID: string, lockDate: Date) => {
        let currentFolder = await utilsFolder.setFolderLock(folderID, lockDate);
        return currentFolder;
    }

    getLockedFolders = async(userID: string) => {
        return await utilsFolder.getLockedFolders(userID);
    }
